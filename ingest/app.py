...
#: The real ControlPlane if the mcpobs_control_plane entry point is
#: registered (ingest/Dockerfile), otherwise control/local.py's
#: single-tenant LocalAuthenticator (ingest/Dockerfile.lite) -- see
#: control/resolve.py. Every call below goes through the Authenticator
#: protocol (control/interfaces.py); nothing here branches on which one it is.
control = _resolve_authenticator()
...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    exactly like a credential problem on their side. `LocalAuthenticator`'s
    `wait_ready`/`migrate` are no-ops, so this is exactly as fast in lite mode
    as calling nothing at all -- it does not need an `if LITE_MODE` guard.
    """
    control.wait_ready()
