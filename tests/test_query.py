...
class TestGenericFilters:
    ...
    def test_attr_condition_is_an_exists_subquery_bound_by_key_and_value(self) -> None:
        """A customer-attached custom attribute (mcpobs.custom.<key> from the
        SDK helper, or any other attribute already in storage) is filterable
        via where=attr.<key>:op:value -- both the key and the value are
        bound parameters, never interpolated."""
        filters = parse("traces", {}, where=["attr.mcpobs.custom.request_id:is:abc-123"])
        params: dict[str, object] = {}
        clauses = filters.clauses(WHERE, params)
        assert len(clauses) == 1
        assert "trace_id IN (SELECT trace_id FROM spans_raw WHERE" in clauses[0]
        assert "tenant_id = {tenant:String}" in clauses[0]
        assert "project_id = {project:String}" in clauses[0]
        assert "timestamp >= {since:DateTime}" in clauses[0]
        assert "span_attributes[{f_c0_key:String}] = {f_c0_val:String}" in clauses[0]
        assert params["f_c0_key"] == "mcpobs.custom.request_id"
        assert params["f_c0_val"] == "abc-123"

    def test_attr_condition_op_variants(self) -> None:
        for raw, expect_fragment in [
            (
                "attr.k:is_not:v",
                "mapContains(span_attributes, {f_c0_key:String}) AND "
                "span_attributes[{f_c0_key:String}] != {f_c0_val:String}",
            ),
            (
                "attr.k:contains:v",
                "positionCaseInsensitive(span_attributes[{f_c0_key:String}], "
                "{f_c0_val:String}) > 0",
            ),
        ]:
            filters = parse("traces", {}, where=[raw])
            clauses = filters.clauses(WHERE, {})
            assert expect_fragment in clauses[0], raw

    def test_attr_condition_is_not_requires_the_key_to_be_present(self) -> None:
        """A trace that never set the attribute at all must not match
        `is_not` -- ClickHouse's Map subscript returns '' for a missing key
        (String's type default), not NULL, so a bare != silently treated
        "never set" the same as "set to something else"."""
        filters = parse("traces", {}, where=["attr.mcpobs.custom.status:is_not:success"])
        clauses = filters.clauses(WHERE, {})
        assert "mapContains(span_attributes, {f_c0_key:String})" in clauses[0]

    def test_attr_condition_key_and_value_are_bound_not_interpolated(self) -> None:
        """A key or value shaped like SQL must end up as a bound value, same
        invariant every other filter in this file already holds."""
        filters = parse("traces", {}, where=["attr.a'; DROP:is:b'; DROP"])
        params: dict[str, object] = {}
        clauses = filters.clauses(WHERE, params)
        assert "DROP" not in clauses[0]
        assert params["f_c0_key"] == "a'; DROP"
        assert params["f_c0_val"] == "b'; DROP"

    def test_attr_condition_is_dropped_on_the_capabilities_view(self) -> None:
        """attr. only makes sense where a trace_id exists to check against --
        capabilities aggregates by name across the window, not per-trace.
        Invalid rows are skipped, not rejected (matches every other filter)."""
        filters = parse("capabilities", {}, where=["attr.request_id:is:abc-123"])
        assert filters.clauses(WHERE, {}) == []

    def test_attr_condition_with_empty_key_is_dropped(self) -> None:
        filters = parse("traces", {}, where=["attr.:is:abc-123"])
        assert filters.clauses(WHERE, {}) == []
