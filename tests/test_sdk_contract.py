...
def build_server() -> MCPServer:
    ...
    @server.tool()
    async def attaches_custom_attribute(request_id: str = "unset") -> str:
        """Calls set_custom_attribute from inside its own handler."""
        from mcpobs import set_custom_attribute

        set_custom_attribute("request_id", request_id)
        return "ok"
...
class TestSpanAnnotation:
    ...
    async def test_span_carries_the_failure_kind(self) -> None:
        ...
        import mcp.shared._otel as _otel_mod
        ...
        # to itself.
        #
        # THE TRAP THAT USED TO ONLY SPRING ONCE ANOTHER TEST SET A PROVIDER,
        # now closed rather than just noted: `mcp.shared._otel` resolves its
        # tracer ONCE at module import (`_tracer = get_tracer("mcp-python-sdk")`)
        # via OTel's proxy mechanism, which locks onto whichever provider is
        # active the FIRST time a span actually gets created through it --
        # not whichever provider is active on any LATER call. Swapping
        # `trace._TRACER_PROVIDER` alone is not enough once any earlier test
        # in the same process has already exercised a real span: this test's
        # own spans would silently go to that earlier, by-then-defunct
        # exporter instead of this one. Verified directly: without also
        # resetting `_otel_mod._tracer`, this exact test passes when run
        # first and fails (0 spans captured, no error) when run after
        # anything else that already resolved the module's tracer once.
        previous = trace._TRACER_PROVIDER
        previous_once = trace._TRACER_PROVIDER_SET_ONCE
        previous_tracer = _otel_mod._tracer
        trace._TRACER_PROVIDER = None
        trace._TRACER_PROVIDER_SET_ONCE = Once()
        trace.set_tracer_provider(provider)
        _otel_mod._tracer = trace.get_tracer("mcp-python-sdk")
        try:
            async with Client(build_server()) as client:
            ...
            _otel_mod._tracer = previous_tracer


class TestCustomAttribute:
    """A customer's own set_custom_attribute call must reach the same span,
    namespaced so it can never collide with mcpobs's own reserved attributes
    (mcpobs.failure.kind and friends, which the classifier trusts
    unconditionally -- see mcpobs/custom.py's module docstring)."""

    async def test_lands_on_the_call_span_under_the_custom_namespace(self) -> None:
        from opentelemetry.sdk.trace import TracerProvider
        from opentelemetry.sdk.trace.export import SimpleSpanProcessor
        from opentelemetry.sdk.trace.export.in_memory_span_exporter import InMemorySpanExporter
        ...
        # See TestSpanAnnotation's comment: resetting _TRACER_PROVIDER alone
        # is not enough once any earlier test already resolved
        # mcp.shared._otel's module-level tracer against a different
        # provider -- it must be re-resolved here too, or this test's spans
        # silently go nowhere this exporter can see.
        previous = trace._TRACER_PROVIDER
        previous_once = trace._TRACER_PROVIDER_SET_ONCE
        previous_tracer = _otel_mod._tracer
        trace._TRACER_PROVIDER = None
        ...
        try:
            async with Client(build_server()) as client:
                await client.call_tool(
                    "attaches_custom_attribute", {"request_id": "test-abc-123"}
                )
            annotated = [
                s
                for s in exporter.get_finished_spans()
                if "mcpobs.custom.request_id" in (s.attributes or {})
            ]
            assert annotated, "set_custom_attribute did not reach the call's span"
            assert annotated[0].attributes["mcpobs.custom.request_id"] == "test-abc-123"
            # Never the raw key -- collision-proofing is the entire point.
            assert "request_id" not in (annotated[0].attributes or {})
        finally:
            trace._TRACER_PROVIDER = previous
            trace._TRACER_PROVIDER_SET_ONCE = previous_once
            _otel_mod._tracer = previous_tracer

    def test_no_active_span_is_a_silent_no_op(self) -> None:
        """Called outside any tool handler -- must not raise."""
        from mcpobs import set_custom_attribute

        set_custom_attribute("request_id", "no-span-here")
