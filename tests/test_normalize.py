...
class TestHttpDownstreamColumns:
    ...
    def test_response_body_column_exists_and_is_honestly_asymmetric(self) -> None:
        """015 re-added this column after 011 dropped it -- read both
        migrations' comments before touching this test again.

        011 was right that the PYTHON hook can never populate it: httpx's span
        closes before `Client.send` reads the body. 015 is not undoing that --
        it's the JS SDK's own attribute (`mcpobs.http.response.body`), which
        that SDK measured it CAN populate (Node's client span stays open
        through the response stream). A Python-sourced span attaching this key
        would be a bug in Python's http.py, not something this normalizer
        should promote -- and it never will, because http.py has no code path
        that stamps it.
        """
        from normalizer.models import DecodedSpan, SpanRow
        from normalizer.normalize import SpanNormalizer

        assert "http_response_body" in SpanRow.model_fields

        js_span = DecodedSpan(
            trace_id="a" * 32, span_id="b" * 16,
            span_attributes={"mcpobs.http.response.body": '{"ok": true}'},
        )
        assert SpanNormalizer().to_row(js_span).http_response_body == '{"ok": true}'

        python_span = DecodedSpan(
            trace_id="a" * 32, span_id="b" * 16,
            span_attributes={"mcpobs.http.request.body": '{"q": 1}'},
        )
        assert SpanNormalizer().to_row(python_span).http_response_body == ""
