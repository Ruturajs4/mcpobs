...
security boundary that is only checked when the whole stack is up is a
boundary that stops being checked the first time someone is in a hurry.

ControlPlane/admin-surface tests (authentication caching, the admin API,
audit logging) moved to the private ECC repo along with control/repository.py
and query/admin.py -- see docs/decisions.md D180. What remains here tests
code that stays in OSS: key minting, request stamping, the archive, SDK
instrumentation, streaming spans, and control/quota.py's QuotaEnforcer
(which needed no split at all -- also D180).
"""
