...
| DF-20 | **A subscription is invisible until it ends, and we see nothing that happens on it.** | Measured against a real `subscriptions/listen` stream: we get exactly ONE span, exported when the stream CLOSES, carrying `mcp.method.name`, `mcp.protocol.version` and `jsonrpc.request.id` -- and nothing else. Three consequences. (1) A subscription open for six hours produces nothing for six hours, so \"is this subscription alive right now?\" -- the first operational question anyone asks -- cannot be answered at all. (2) Events delivered on the stream emit no spans, so throughput and starvation are invisible. (3) `io.modelcontextprotocol/subscriptionId` and the honored `SubscriptionFilter` are not captured, so one stream cannot be told from another. This is inherent to modelling a long-lived stream as a single span; closing it needs a deliberate mechanism (periodic heartbeat spans, or metrics rather than spans), not a bigger attribute list. | A design partner running subscriptions in anger, or the first support question about one. | OPEN |
| DF-21 | **Progress notifications produce no telemetry at all.** | Measured: a tool that called `ctx.report_progress()` three times, with a client that supplied a `progressToken` and received all three, produced ONE span of 155ms and nothing else. So for a long-running tool we can say it is running and not how far along it is -- which is the entire question progress exists to answer, and the one an operator asks about a job that has been going for an hour. Same root cause as DF-20: a single span cannot report on itself while it is open. Fixing it needs a mechanism (span events at each report, or progress as a metric), not another attribute. `progressToken` now survives redaction (D113), so the correlation key at least exists. | A design partner with a long-running tool, or the first \"is it stuck?\" support question. | OPEN |
| DF-22 | **OAuth is completely invisible: no auth span has ever been stored.** | Authorization is a TRANSPORT-level concern in MCP, so it resolves before any method dispatch -- before the SDK's OTel middleware and before ours. A client that fails to authenticate produces a 401 and NO MCP span at all, so the entire class of \"my clients cannot connect\" is absent from the console rather than shown as an error. That includes expired tokens, wrong-audience tokens, failed discovery against `/.well-known/oauth-protected-resource`, and 403 `insufficient_scope` -- and the last one is not even a misconfiguration: the spec's step-up authorization flow makes it a ROUTINE runtime occurrence that clients are expected to handle. We are blind to a normal protocol flow, and to the first question a customer asks when onboarding fails. Fixing it means observing the HTTP layer beneath MCP (ASGI instrumentation on the server, or the gateway reporting 401/403 by tenant), which is a different data source, not a missing attribute. | The first customer whose clients cannot authenticate. | OPEN |
