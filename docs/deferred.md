...
| DF-C11 | Session tokens for client-launched (stdio) servers (was DF-25) | **Closed.** ADR-011 built end to end: verification in the ingest gateway, a `POST /v1/sessions` mint endpoint behind its own `session` scope, an SDK provider that refreshes at 75% with jitter, and an OTLP exporter that rotates the credential per export. Proved load-bearing rather than assumed: a stdio server with only a session endpoint exported 6 MCP spans end to end, and the SAME server pointed at a dead endpoint exported **0** -- with a dev key sitting on disk, so it did not quietly fall back. Boundaries measured: a session token ingests (200), cannot read (401), and cannot mint another (401); a mint key cannot ingest (401). |
