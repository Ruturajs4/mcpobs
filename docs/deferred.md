...
| DF-25 | **Session tokens for client-launched (stdio) servers.** ADR-011 accepted, unimplemented. | A stdio server can only be given a long-lived, org-wide ingest key -- and on stdio that key lives in a config file on the END USER's machine, which is roughly half of MCP deployments. The exposure is not that a key can leak but that nothing expires when it does: quota exhaustion, poisoned error rates and fabricated servers in the customer's console, indefinitely. | The first design partner shipping a stdio server to users they do not control. | OPEN |
