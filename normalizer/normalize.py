...
class SpanNormalizer:
    ...
    # Only ever stamped by the JS SDK -- Python's httpx-based instrumentation
    # cannot populate this (its span closes before the body is read; see
    # normalizer/schema/015_http_response_body.sql). Empty for Python-sourced
    # spans, by design, not a bug.
    HTTP_RESP_BODY: Final = "mcpobs.http.response.body"
    ...
    def to_row(self, span: DecodedSpan, *, partition: int = -1, offset: int = -1) -> SpanRow:
        ...
        return SpanRow(
            ...
            http_response_body=self._str(attrs.get(self.HTTP_RESP_BODY)),
