-- CLOUD OVERRIDE of schema/001_spans_raw.sql -- identical except for the
-- ENGINE line. ClickHouse Cloud's managed "Replicated" database engine
-- assigns zookeeper_path/replica_name itself and rejects a CREATE TABLE
-- that also specifies them (confirmed against a real Cloud service; see
-- MigrationRunner.override_dir in normalizer/migrations.py). Edit that
-- file's schema/ counterpart for column/engine-family changes and mirror
-- them here -- this file is not an independent source of truth.
--
-- Day-1 span table.
--
