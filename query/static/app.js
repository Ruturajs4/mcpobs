...
/* A collapsible, syntax-highlighted rendering for any value that turns out to
   be JSON -- reused everywhere a JSON-shaped string reaches this panel
   (request/response payloads, HTTP headers, span attribute values), so there
   is one tree renderer rather than a one-off widget per field. */
let jsonTreeSeq = 0;
...
function maybeJson(text) {
  ...
  try {
    ...
    // Capture previews are truncated ("N chars · truncated" in io()
    // below), so a cut-off payload is often syntactically invalid JSON --
    // falls through to the caller's existing plain-text rendering rather
    // than erroring or showing a broken tree.
    return null;
  }
  const id = `jv${++jsonTreeSeq}`;
  return `<div class="json-tree" id="${id}" data-raw="${esc(JSON.stringify(value, null, 2))}">
    <button type="button" class="jv-copy" data-copy-target="${id}">Copy</button>
...
function renderJsonValue(value, depth) {
  ...
  if (typeof value === "string") return `<span class="jv-str">"${esc(value)}"</span>`;
  if (Array.isArray(value)) {
    return renderJsonContainer(
      value.map((v, i) => ({ key: null, value: v, last: i === value.length - 1 })),
      "[", "]", depth,
    );
  }
  if (typeof value === "object") {
    ...
    return renderJsonContainer(
      keys.map((k, i) => ({ key: k, value: value[k], last: i === keys.length - 1 })),
      "{", "}", depth,
    );
  }
...
function renderJsonContainer(entries, open, close, depth) {
  if (!entries.length) return `<span class="jv-punct">${open}${close}</span>`;
  const rows = entries.map(({ key, value, last }) =>
    `<div class="jv-row">${key !== null
      ? `<span class="jv-key">"${esc(key)}"</span><span class="jv-punct">: </span>` : ""
    }${renderJsonValue(value, depth + 1)}${last ? "" : `<span class="jv-punct">,</span>`}</div>`
  ).join("");
  const count = entries.length;
  // Depth < 2 open by default: small payloads stay fully visible on open,
  // deep ones don't turn the panel into a wall of expanded brackets.
  const label = open === "{" ? `${count} key${count === 1 ? "" : "s"}` : `${count} item${count === 1 ? "" : "s"}`;
  return `<details class="jv-node"${depth < 2 ? " open" : ""}>
    <summary class="jv-summary"><span class="jv-punct">${open}</span><span class="jv-count">${label}</span></summary>
    <div class="jv-body">${rows}</div>
    <span class="jv-punct">${close}</span>
  </details>`;
}
...
async function viewOverview(signal) {
  ...
  el("content").innerHTML = `
    ...
    <div class="panel"><header><h3>Status breakdown</h3>
...
const CLIENT_ICON_ASSISTANT =
  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"><path d="M12 2l1.8 6.2L20 10l-6.2 1.8L12 18l-1.8-6.2L4 10l6.2-1.8L12 2z"/></svg>';
...
function renderSpanDetail(d) {
  ...
  const map = (title, obj, note = "") => {
    ...
    const rows = keys.sort().map((k) => {
      const tree = maybeJson(obj[k]);
      // A JSON-shaped value (mcpobs.http.request.headers and friends) gets a
      // full-width block instead of being squeezed into the narrow inline
      // .fv cell -- everything else keeps the compact row exactly as before.
      return tree
        ? `<div class="f f-json"><span class="fk">${esc(k)}</span>${tree}</div>`
        : `<div class="f"><span class="fk">${esc(k)}</span><span class="fv">${esc(obj[k])}</span></div>`;
    }).join("");
    return `<div class="grp"><h4>${title} <span class="mute">${keys.length}</span>
      ${note ? `<span class="note">${note}</span>` : ""}</h4>
      <div class="fields mono">${rows}</div></div>`;
  };
  ...
  //
  // `content` is a string, OR an array whose items are each either a string
  // or a {label, text} pair -- the HTTP-downstream case below needs an info
  // line (method/URL, or status) alongside headers and a body, each of which
  // may independently be JSON. Trying maybeJson() on the three joined into
  // one string always fails (the line prefix isn't JSON), silently losing
  // tree rendering for headers/body that ARE. Each segment gets its own
  // attempt instead. Two JSON segments back to back (headers, then body)
  // render as two visually identical boxes with nothing distinguishing them
  // -- {label, text} items get a small caption above their block so
  // "Headers" and "Body" don't have to be inferred from content alone.
  const io = (label, content, size) => {
    const items = (Array.isArray(content) ? content : [content])
      .map((it) => (typeof it === "string" ? { text: it } : it))
      .filter((it) => it && it.text);
    if (!items.length) return "";
    const body = items.map(({ label: subLabel, text }) => {
      const rendered = maybeJson(text) || `<pre>${esc(text)}</pre>`;
      return subLabel ? `<div class="io-sublabel">${esc(subLabel)}</div>${rendered}` : rendered;
    }).join("");
    const totalLength = items.map((it) => it.text).join("").length;
    return `<div class="io"><div class="io-head">${label}
         <span class="note mono">${size ? `${size} chars` : ""}${
           size && totalLength < size ? " · truncated" : ""}</span></div>
       ${body}</div>`;
  };
  ...
  if (!d.mcp_method) {
    ...
    if (d.downstream_kind === "http") {
      ...
      // The request line, then headers and body under it -- each its own
      // array element so io() can try JSON-tree rendering on headers/body
      // independently of the plain-text line. Showing the body alone would
      ...
      request = [
        reqLine,
        { label: "Headers", text: d.http_request_headers },
        { label: "Body", text: d.http_request_body },
      ];
      response = [
        respLine,
        { label: "Headers", text: d.http_response_headers },
        { label: "Body", text: d.http_response_body },
      ];
      // Response body is SDK-dependent, not universally missing: the JS SDK
      // captures it (mcpobs-js/src/downstream.ts), Python's httpx-based
      // instrumentation cannot (its span closes before the body is read).
      // Both messages are real depending on which produced this span, so say
      // so rather than pointing every viewer at a Python-only function name.
      footnote = d.http_request_headers
        ? "redacted and truncated at capture, credential headers never read" +
          (d.http_response_body ? "" :
            " \u00b7 no response body: either this SDK cannot capture one (Python/httpx), or it was skipped here (binary content-type, or over the size cap)")
        : "request body and headers not captured \u2014 enable HTTP detail capture in your server (captureHttpDetail in the JS SDK, instrument_httpx() in Python)";
  ...
  // request/response are either a plain string (most downstream kinds) or
  // an array of segments (the http kind, above) -- a non-empty ARRAY is
  // always truthy even when every segment inside it is an empty string, so
  // `request || response` alone stopped detecting "nothing captured" for
  // http spans the moment they became arrays. Checked per-shape instead.
  const hasContent = (value) =>
    Array.isArray(value)
      ? value.some((it) => (typeof it === "string" ? it : Boolean(it && it.text)))
      : Boolean(value);

  const payloadBlock = (hasContent(request) || hasContent(response))
  ...
  // Delegated after the fact, matching this file's own convention for
  // dynamically generated content (list rows, filter controls) rather than
  // inline onclick attributes: query the buttons the HTML above just
  // created and wire each to the JSON block it labels.
  el("span-detail").querySelectorAll(".jv-copy").forEach((btn) => {
    btn.addEventListener("click", () => {
      const raw = el(btn.dataset.copyTarget)?.dataset.raw;
