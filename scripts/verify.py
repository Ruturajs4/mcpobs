...
# docker-compose.yml moved to the private ECC repo in the OSS/ECC split
# (docs/decisions.md D180) -- `compose()` below needs to run against
# wherever that file actually lives now, which is no longer REPO_ROOT.
# Defaults to REPO_ROOT (this script's own historical behavior, unchanged
# for anyone who still has the pre-split layout) so this is additive, not a
# behavior change for existing callers.
COMPOSE_DIR = Path(os.environ.get("MCPOBS_VERIFY_COMPOSE_DIR", REPO_ROOT)).resolve()
